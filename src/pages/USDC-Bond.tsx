          ...
          <section className="flex flex-col-reverse lg:flex    lg:flex-row  lg:me-[25vh] lg:mx-[8vh] font-manrope lg:max-w-[1000px] ">
            <div className=" lg:min-w-[445px]  flex items-center justify-center ">
              <div className="flex flex-col lg:flex-row lg:flex-wrap lg:items-center lg:justify-center lg:space-y-3 space-y-4 mt-10 lg:0">
                <Card
                  title="Bond Price"
                  value={formatterUSD.format(parseFloat(ethers.formatUnits(bondShareData.bondPrice, 6)))}
                  conversion={null}
                />

                <Card title="Discount" value={`${parseFloat(bondShareData.discount.toString()).toFixed(2)}%`} conversion={null} />
                <Card
                  title="Remaining Bonds"
                  value={formatter.format(parseFloat(ethers.formatUnits(bondShareData.remainingBrr, 9)))}
                  conversion={null}
                />
              </div>
            </div>
