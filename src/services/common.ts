import { AddressLike, ContractTransactionResponse, ethers } from "ethers";
...
export const approveContract = async (
  ...
  try {
    const tx: ContractTransactionResponse = await tokenContract.approve(spender, amount);
 ...
 */
export const getUserBalance = async (tokenContract: ethers.Contract | Erc20, user: AddressLike): Promise<bigint | null> => {
  try {
 ...
 */
export const getTokenPrice = async (tokenContract: ethers.Contract | BeraReserveOlympusERC20Token): Promise<bigint | null> => {
  try {
 ...
 */
export const getMarketCap = async (tokenContract: ethers.Contract | BeraReserveOlympusERC20Token): Promise<bigint | null> => {
  try {
...
export const getBondPriceInUSD = async (bondDepository: ethers.Contract | BeraReserveBondDepository): Promise<bigint | null> => {
  try {
...
export const getTotalStaked = async (stakingContract: ethers.Contract | BeraReserveStaking): Promise<bigint | null> => {
  try {
...
export const calculateAPR = async (sBeraContract: ethers.Contract | BeraReserveSOlympusERC20Token): Promise<number | null> => {
  try {
    ...
    console.log("calculateEffectiveAPR", calculateEffectiveAPR(parseFloat(formattedRebase)));
...
function calculateNumberOfBlocksPerDay(averageBlockTimeInSeconds: number = 12.07) {
