...
import { Erc20, approveContract, checkAllowance, useBeraReserveContract } from "../../../services";
...
const StakeCard = () => {
  ...
  const handleOnMaxAmountClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setDisplayStake(parseFloat(ethers.formatUnits(stakeData.brrBalance, 9)).toString());
    setStakeAmount(BigInt(ethers.parseUnits(parseFloat(ethers.formatUnits(stakeData.brrBalance, 9)).toString(), 9)));
  };

  const { bRStakingContract, bRTokenContract } = useBeraReserveContract();

  const handleClaimsBRR = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();

    if (!bRStakingContract || !userAddress) {
    ...
    try {
      const txHashClaim = await bRStakingContract?.claim(userAddress as AddressLike);
  ...
  const handleStakeBRR = async (e: { preventDefault: () => void }) => {
    e.preventDefault();
    if (!bRStakingContract || !bRTokenContract || !userAddress) {
      showToast("Error fetching contract", "failed");
      return;
    }
    ...
    if (stakeAmount < minimumStakeAmount) {
      ...
      showToast(`Minimum Stake Amount: ${ethers.formatUnits(minimumStakeAmount, 9)} BRR`, "failed");
      return;
    }
    ...
    if (!isApproved) {
      const txHash = await approveContract(bRTokenContract as Erc20, bRStakingContract as AddressLike, stakeAmount);
      if (!txHash) {
    ...
    showToast(`Staking Initiated: ${Number(ethers.formatUnits(stakeAmount, 9)).toFixed(3)} BRR`, "pending");

    try {
      const txHash = await bRStakingContract?.stake(stakeAmount, userAddress as AddressLike);
  ...
  useEffect(() => {
    checkAllowance(bRTokenContract as Erc20, userAddress as AddressLike, bRStakingContract as AddressLike, stakeAmount).then(
      (res) => {
        res ? setIsApproved(res) : setIsApproved(false);
      }
    );
  }, [bRTokenContract, stakeAmount, bRStakingContract, userAddress]);
  return (
    <>
        ...
        <div className="flex-col justify-center items-center  inline-flex  text-center lg:min-w-[400px]  p-6   ">
          ...
          <form action="#" className=" w-full ">
            <div className="">
                  ...
                  <div className="">
                    <span className="text-yellow-400 text-xs font-normal font-['Manrope'] leading-none inline-block">
                      ...
                      <span className="text-white">
                        {formatter.format(parseFloat(ethers.formatUnits(stakeData.brrBalance, 9)))} BRR{" "}
                      </span>
                    </span>
                    <button
              ...
              <PrimaryButton buttonText="Stake" onBuyBond_StakeClick={handleStakeBRR} />

              <SecondaryButton buttonText="Claim sBRR" onClaimBondClick={handleClaimsBRR} />
            </div>
          </form>
          <div className=" flex-col w-full mt-3 space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-yellow-400 text-sm font-normal font-['Manrope'] leading-tight">Staked Balance</span>
              <div className="text-white text-sm font-semibold font-['Manrope'] leading-tight flex space-x-2">
                <span>{formatter.format(parseFloat(ethers.formatUnits(stakeData.amountStaked, 9)))} BRR</span>
              </div>
            </div>
          </div>
        </div>
