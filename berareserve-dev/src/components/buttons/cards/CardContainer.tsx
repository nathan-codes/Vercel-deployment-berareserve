...
const CardContainer: React.FC<CardContainerProps> = ({ bondType }) => {
  ...
  const handleOnMaxAmountClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setDisplayCollateral(
      parseFloat(ethers.formatUnits(bondInfo.balance, decimals)).toString()
    );
  ...
  const handleBuyBond = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const minimumBondAmount: bigint =
      bondType === "USDC" ? 1_000_000n : 1_000_000_000_000_000_000n;

    if (collateralBigint < minimumBondAmount) {
      //show toast of min amount
      showToast(
        `Minimum Bond Amount: $${ethers.formatUnits(
          minimumBondAmount,
          decimals
        )}`,
        "failed"
    ...
    if (!isApproved) {
      const txHash = await approveContract(
        bondType === "USDC" ? (usdcContract as Erc20) : (lpContract as Erc20),
    ...
    showToast(
      `Bond Purchase for : $${Number(
        ethers.formatUnits(collateralBigint, decimals)
      ).toFixed(2)}`,
      "pending"
    );

    try {
      const txHash = await bondContract?.deposit(
        collateralBigint as BigNumberish,
        maxPriceToBuyBond,
  ...
  const handleClaimBRR = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const sanitized: number = parseFloat(
      bondInfo.amountBonded.replace(/[^0-9.-]/g, "")
    );
    ...
    try {
      const txHash = await bondContract?.redeem(
        userAddress as AddressLike,
        false
      );
      ...
      if (receipt?.status === 1) {
        ...
        const formattedClaimableAmount = formatter.format(
          parseFloat(ethers.formatUnits(claimableAmount, 9))
        );
    ...
    <>
        ...
        <div className="  flex-col justify-center items-center  inline-flex  text-center  lg:min-w-[450px] w-[340px] lg:p-10  ">
          <form action="#" className=" w-full ">
            <div className="">
              ...
              <PrimaryButton
                buttonText="Bond"
                onBuyBond_StakeClick={(e) => handleBuyBond(e)}
              />

              <SecondaryButton
                buttonText="Claim BRR"
                onClaimBondClick={(e) => handleClaimBRR(e)}
              />
            </div>
          </form>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-white text-sm font-semibold font-['Manrope'] leading-tight flex space-x-2">
                <span>
                  {" "}
                  {formatterUSD.format(
                    parseFloat(ethers.formatUnits(bondInfo.balance, decimals))
                  )}{" "}
                  {currencyDisplay}
                </span>{" "}
      ...
      {isRedeemModalOpen && (
        <MintNRedeemModal
          ...
          currency="BRR"
          closeModal={() => setIsRedeemModalOpen(false)}
        />
      )}
    </>
  );
